...
def sim(z_i, z_j):
    ...
    leni = torch.linalg.norm(z_i)
    lenj = torch.linalg.norm(z_j)
    norm_dot_product = z_i.dot(z_j) / (leni * lenj)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N): 
            if i != k:
                x += torch.exp(sim(z_k, out[i]) / tau) 
            if i != k + N: 
                y += torch.exp(sim(z_k_N, out[i]) / tau)
        tmp = torch.exp(sim(z_k, z_k_N) / tau) 
        total_loss += - torch.log(tmp / x) - torch.log(tmp / y)
...
def sim_positive_pairs(out_left, out_right):
    ...
    len_l = torch.linalg.norm(out_left, dim = 1, keepdim = True) 
    len_r = torch.linalg.norm(out_right, dim = 1, keepdim = True) 
    pos_pairs = (out_left * out_right).sum(axis = 1, keepdim = True) / (len_l * len_r) 
...
def compute_sim_matrix(out):
    ...
    len_o = torch.linalg.norm(out, dim = 1, keepdim = True) 
    nout = out / len_o 
    sim_matrix = torch.matmul(nout, nout.T) 
