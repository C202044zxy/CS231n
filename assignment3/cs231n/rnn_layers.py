...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(np.matmul(x, Wx) + np.matmul(prev_h, Wh) + b.reshape(1, -1))
...
def rnn_step_backward(dnext_h, cache):
    ...
    s = np.matmul(x, Wx) + np.matmul(prev_h, Wh) + b.reshape(1, -1)
    ds = 4 / ((np.exp(s) + np.exp(-s))**2) * dnext_h 
...
def rnn_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dx[:, i, :], dprev_h, tdWx, tdWh, tdb = rnn_step_backward(dprev_h + dh[:, i, :], cache[i])
        dWx += tdWx 
        dWh += tdWh 
        db += tdb 
    dh0 = dprev_h
...
def word_embedding_forward(x, W):
    ...
    # transform the word into vector. 
    N, T = x.shape 
    V, D = W.shape 
    out = np.zeros((N, T, D))
    for i in range(N): 
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    N, H = prev_h.shape 
    A = np.matmul(prev_h, Wh) + np.matmul(x, Wx) + b # (N, 4 * H)
    i = sigmoid(A[:, 0 : H])  # (N, H)
    f = sigmoid(A[:, H : 2*H]) # (N, H)
    o = sigmoid(A[:, 2*H : 3*H]) # (N, H)
    g = tanh(A[:, 3*H : 4*H]) # (N, H)
    next_c = f * prev_c + i * g 
    next_h = o * tanh(next_c)
    cache = (next_c, next_h, x, prev_h, prev_c, Wx, Wh, b, i, f, o, g, A)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    (next_c, next_h, x, prev_h, prev_c, Wx, Wh, b, i, f, o, g, A) = cache 
    do = tanh(next_c) * dnext_h * dsigmoid(o)
    dnext_c += o * dnext_h * dtanh(tanh(next_c)) # add the gradient in the computational graph 
    dprev_c = f * dnext_c 
    df = prev_c * dnext_c * dsigmoid(f)
    di = g * dnext_c * dsigmoid(i)
    dg = i * dnext_c * dtanh(g)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache_lstm = []
    for i in range(T):
        h0, prec, tmp = lstm_step_forward(x[:, i], h0, prec, Wx, Wh, b) 
        h[:, i] = h0 # store h 
        cache_lstm.append(tmp) 
    cache = (x, h0, Wx, Wh, b, cache_lstm)
...
def lstm_backward(dh, cache):
    ...
    dc = np.zeros((N, H)) 
    for i in range(T - 1, -1, -1): 
        tdx, dh0, dc, tdWx, tdWh, tdb= lstm_step_backward(dh[:, i] + dh0, dc, cache_lstm[i]) 
        dx[:, i] = tdx 
        dWx += tdWx 
        dWh += tdWh 
        db += tdb 
