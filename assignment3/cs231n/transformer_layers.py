...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.n_head 
        key = self.key(key).view(N, T, H, -1).swapaxes(1, 2) # (N, H, T, E / H) 
        value = self.value(value).view(N, T, H, -1).swapaxes(1, 2) # (N, H, T, E / H)
        query = self.query(query).view(N, S, H, -1).swapaxes(1, 2) # (N, H, S, E / H)
        score = torch.matmul(query, key.swapaxes(2, 3)) / math.sqrt(E / H) # (N, H, S, T) 
        if attn_mask is not None: 
            score.masked_fill(~attn_mask, -math.inf) # fill with -inf where attn_mask = 1
        score = nn.functional.softmax(score, dim = -1) # long the last dimension 
        score = self.attn_drop(score)
        y = torch.matmul(score, value) # (N, H, S, E/H)
        y = y.swapaxes(1, 2) 
        y = y.reshape(N, S, E) 
        output = self.proj(y)
