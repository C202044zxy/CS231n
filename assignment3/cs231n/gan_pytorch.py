...
def sample_noise(batch_size, dim, seed=None):
    ...
    x = torch.rand(batch_size, dim) * 2 - 1
    return x
...
def discriminator_loss(logits_real, logits_fake):
    ...
    score = torch.cat([logits_real, logits_fake]).squeeze()
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = torch.mean((scores_real - 1) ** 2) * 0.5
    loss += torch.mean(scores_fake ** 2) * 0.5
...
def ls_generator_loss(scores_fake):
    ...
    loss = torch.mean((scores_fake - 1) ** 2) * 0.5
